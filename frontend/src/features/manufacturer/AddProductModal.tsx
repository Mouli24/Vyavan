...
export default function AddProductModal({ open, onClose, onPublished, editProduct }: Props) {
  ...
  async function handlePublish(draft = false) {
    ...
    try {
      ...
      // Filter out base64 images (too large for API) — only keep URL strings
      const validPhotos = form.photos.filter(p => p.startsWith('http') || p.startsWith('https'))
      const payload = {
        name: form.name,
        ...
        image: validPhotos[0] ?? form.photos[0] ?? '',
        isActive: !draft,
        description: form.description,
        shortDescription: form.description?.slice(0, 150),
        sku: form.sku,
        sector: form.sector,
        unit: form.unitType,
        unitsPerPack: form.unitsPerPack,
        photos: validPhotos.length > 0 ? validPhotos : form.photos.slice(0, 1),
        specs: specsObj,
      ...
      setPublishError(e?.message ?? 'Failed to publish product. Please try again.')
    } finally {
      setPublishing(false)
    }
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-2">Product Code / SKU</label>
          <input
            type="text"
            value={form.sku}
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 mb-2">Sector Served</label>
          <select
            value={form.sector}
          ...
          >
            ...
            {['Retail', 'Food', 'Pharma', 'Packaging', 'Hardware'].map(s => <option key={s} value={s}>{s}</option>)}
          </select>
        </div>
      </div>
    ...
    <div className="flex flex-col gap-6">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          >
            {['Piece', 'Packet', 'Box', 'Bag', 'Bora', 'Kg', 'Litre', 'Roll'].map(u => <option key={u} value={u}>{u}</option>)}
          </select>
        </div>
        <div>
          ...
          <input 
            ...
            placeholder="e.g. 1 box = 24 packets"
            className="w-full px-4 py-2.5 rounded-xl border border-slate-200 text-sm" 
          />
        </div>
      </div>
...
function Step7({ form, publishing, published, publishError, onDraft, onPublish }: any) {
  if (published) {
    return (
    ...
    <div className="flex flex-col gap-6">
      <div className="bg-slate-50 rounded-3xl p-6 border border-slate-200">
        <div className="flex gap-4">
          <div className="w-32 h-32 rounded-xl bg-white border border-slate-200 overflow-hidden flex items-center justify-center">
            {mainPhoto ? <img src={mainPhoto} className="w-full h-full object-cover" /> : <ImageIcon size={32} className="text-slate-300" />}
          </div>
          <div>
      ...
      <div className="flex gap-4">
        <button onClick={onDraft} disabled={publishing} className="flex-1 py-3 rounded-xl border-2 border-slate-200 font-bold hover:bg-slate-50 disabled:opacity-50">Draft</button>
        <button onClick={onPublish} disabled={publishing} className="flex-[2] py-3 rounded-xl bg-[#5D4037] text-white font-bold hover:bg-[#4E342E] flex items-center justify-center gap-2 disabled:opacity-70">
