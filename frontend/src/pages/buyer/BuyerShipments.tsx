...
function transportColor(type: string) {
  const map: Record<string, string> = {
    ...
    train_parcel: 'bg-purple-100 text-purple-700',
...
function ShipmentDetails({ s }: { s: any }) {
  ...
  const Row = ({ icon: Icon, label, value, href }: any) =>
    value ? (
      <div className="flex items-center gap-3 py-3 border-b border-sp-border last:border-0">
        <div className="w-8 h-8 bg-sp-bg rounded-lg flex items-center justify-center shrink-0">
          <Icon size={14} className="text-sp-purple" />
        </div>
        <div className="flex-1 min-w-0">
          <p className="text-[10px] font-black text-sp-muted uppercase tracking-widest">{label}</p>
          {href ? (
            <a href={href} target="_blank" rel="noopener noreferrer"
              className="text-sm font-bold text-sp-purple hover:underline flex items-center gap-1">
              {value} <ExternalLink size={11} />
            </a>
          ) : (
            <p className="text-sm font-bold text-sp-text truncate">{value}</p>
          )}
        </div>
      </div>
    ) : null;
  ...
  if (type === 'own_vehicle' || type === 'transport_company') return (
    <div>
      <p className="text-xs font-black text-sp-muted uppercase tracking-widest mb-3">
  ...
  if (type === 'bus_cargo') return (
    <div>
      <p className="text-xs font-black text-sp-muted uppercase tracking-widest mb-3">Bus Cargo Details</p>
  ...
  if (type === 'train_parcel') return (
    <div>
      <p className="text-xs font-black text-sp-muted uppercase tracking-widest mb-3">Train Parcel Details</p>
  ...
  if (type === 'courier') return (
    <div>
      <p className="text-xs font-black text-sp-muted uppercase tracking-widest mb-3">Courier Details</p>
      ...
      {s.trackingUrl && (
        <a href={s.trackingUrl} target="_blank" rel="noopener noreferrer"
          className="mt-3 flex items-center justify-center gap-2 w-full py-3 rounded-xl bg-sp-purple text-white text-xs font-black uppercase tracking-widest hover:opacity-90 transition-all">
  ...
  return (
    <div className="min-h-screen bg-sp-bg">
      ...
      <div className="mb-10 flex items-center justify-between flex-wrap gap-4">
        <div>
          <h1 className="text-4xl font-black text-sp-text tracking-tight">
            Track <span className="text-sp-purple">Shipments</span>
          </h1>
          <p className="text-sp-muted font-bold uppercase text-[10px] tracking-widest mt-1">Real-time logistics monitoring</p>
        </div>
          ...
          {['all', 'transit', 'delivered'].map(f => (
            <button
              ...
              className={`px-5 py-2.5 rounded-full text-xs font-black uppercase tracking-widest transition-all ${
                filter === f ? 'gradient-card-purple text-white' : 'bg-sp-bg text-sp-muted hover:bg-sp-purple-pale hover:text-sp-purple'
              }`}
            >
      ...
      {filtered.length === 0 ? (
        <div className="bg-sp-bg rounded-[2.5rem] py-20 text-center border-2 border-dashed border-sp-border">
          ...
          <p className="font-black text-sp-text">No shipments found</p>
          <p className="text-xs text-sp-muted mt-1 uppercase font-bold tracking-widest max-w-xs mx-auto">
            ...
            {filtered.map(s => {
              ...
              return (
                <motion.div
                  ...
                  className="bg-white rounded-[2.5rem] p-8 border border-sp-border shadow-sm hover:shadow-hover transition-all group cursor-pointer"
                >
                  {/* Top row */}
                  <div className="flex items-start justify-between gap-4 mb-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <h3 className="text-xl font-black text-sp-text">{s.shipmentId}</h3>
                        <p className="text-xs font-bold text-sp-muted uppercase tracking-widest mt-0.5">
                          {transportLabel(s.transportType)}
                          {relOrders.length > 1 && ` · ${relOrders.length} combined orders`}
                        </p>
                        {relOrders.map((o: any) => (
                          <span key={o._id || o} className="inline-block mr-2 mt-1 text-[10px] font-bold bg-sp-bg text-sp-text px-2 py-0.5 rounded-full">
                            {o.orderId || o}
                          </span>
                        ))}
                      </div>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      ...
                      <button className="w-10 h-10 rounded-full bg-sp-bg flex items-center justify-center text-sp-muted group-hover:bg-sp-purple group-hover:text-white transition-all">
                  ...
                  <div className="relative flex items-center justify-between mb-6">
                    <div className="absolute left-0 right-0 top-4 h-0.5 bg-sp-border -z-0" />
                    <div
                      className="absolute left-0 top-4 h-0.5 bg-sp-purple -z-0 transition-all duration-700"
                      style={{ width: `${(stepIdx / (ORDER_STEPS.length - 1)) * 100}%` }}
                    />
                    {ORDER_STEPS.map((step, i) => (
                      <div key={step} className="flex flex-col items-center gap-2 z-10">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${
                          i < stepIdx ? 'bg-sp-purple border-sp-purple' :
                          i === stepIdx ? 'bg-sp-purple border-sp-purple ring-4 ring-sp-purple/20' :
                        ...
                        <span className={`text-[9px] font-black uppercase tracking-widest whitespace-nowrap ${
                          i <= stepIdx ? 'text-sp-purple' : 'text-sp-muted'
                        }`}>
                          {step}
                        </span>
                      </div>
                  ...
                  <div className="flex flex-wrap gap-3 text-xs">
                    {s.arrival && (
                      <span className="flex items-center gap-1.5 bg-sp-bg px-3 py-1.5 rounded-full font-bold text-sp-text">
                        <Calendar size={12} className="text-sp-purple" /> {s.arrival}
                      </span>
                    )}
                    {(s.transportType === 'own_vehicle' || s.transportType === 'transport_company') && s.driverPhone && (
                      <span className="flex items-center gap-1.5 bg-sp-bg px-3 py-1.5 rounded-full font-bold text-sp-text">
                        <Phone size={12} className="text-sp-purple" /> {s.driverPhone}
                      </span>
                    )}
                    {s.transportType === 'bus_cargo' && s.parcelReceiptNumber && (
                      <span className="flex items-center gap-1.5 bg-sp-bg px-3 py-1.5 rounded-full font-bold text-sp-text">
                        <Barcode size={12} className="text-sp-purple" /> {s.parcelReceiptNumber}
                      </span>
                    )}
                    {s.transportType === 'train_parcel' && s.parcelBookingNumber && (
                      <span className="flex items-center gap-1.5 bg-sp-bg px-3 py-1.5 rounded-full font-bold text-sp-text">
                        <Barcode size={12} className="text-sp-purple" /> {s.parcelBookingNumber}
                      </span>
                    )}
                    {s.transportType === 'courier' && s.trackingNumber && (
                      <span className="flex items-center gap-1.5 bg-sp-bg px-3 py-1.5 rounded-full font-bold text-sp-text">
                        <Package size={12} className="text-sp-purple" /> {s.carrier} · {s.trackingNumber}
                      </span>
                    )}
      ...
      <AnimatePresence>
        {activeShipment && (
          ...
          >
            ...
            >
              {/* Modal Header */}
              <div className="p-6 border-b border-sp-border flex justify-between items-start shrink-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h3 className="font-black text-xl text-sp-text">{activeShipment.shipmentId}</h3>
                    <p className="text-xs text-sp-muted font-bold uppercase tracking-widest">
                      {transportLabel(activeShipment.transportType)} · {activeShipment.status}
                    </p>
                  </div>
                </div>
                <button onClick={() => setActiveShipment(null)} className="w-9 h-9 bg-sp-bg rounded-full flex items-center justify-center text-sp-muted hover:text-sp-text">
                  <X size={18} />
                </button>
              </div>
              ...
              <div className="overflow-y-auto flex-1 p-6 space-y-6">
                ...
                <div className="bg-sp-bg rounded-2xl p-5">
                  <ShipmentDetails s={activeShipment} />
                </div>

                {/* Tracking Timeline */}
                <div>
                  <h4 className="text-xs font-black text-sp-muted uppercase tracking-widest mb-4 flex items-center gap-2">
                    <Clock size={13} /> Live Updates
                  </h4>
                  {trackingLoading ? (
                    <div className="flex justify-center py-8">
                      <Loader2 className="w-7 h-7 animate-spin text-sp-purple" />
                    </div>
                  ) : trackingEvents.length === 0 ? (
                    <p className="text-center text-sp-muted py-8 font-bold tracking-widest uppercase text-xs">No carrier updates yet.</p>
                  ) : (
                    <div className="relative border-l-2 border-sp-purple/20 ml-3 space-y-6 pb-2">
                      {trackingEvents.map((ev: any, i: number) => (
                        <div key={i} className="relative pl-7">
                          <div className={`absolute -left-[9px] top-1 w-4 h-4 rounded-full border-4 border-white shadow-sm ${i === 0 ? 'bg-sp-purple' : 'bg-sp-border'}`} />
                          <p className="font-black text-sp-text text-sm mb-0.5 uppercase tracking-wide">{ev.status}</p>
                          <p className="text-xs text-sp-muted mb-1.5">{ev.message}</p>
                          <div className="flex items-center gap-4 text-[10px] font-bold text-sp-muted uppercase tracking-widest">
                ...
                {activeShipment.status !== 'Delivered' && (
                  <div className="bg-sp-bg rounded-2xl p-5 border border-sp-border">
                    <p className="text-sm font-black text-sp-text mb-1">Received your order?</p>
                    <p className="text-xs text-sp-muted mb-4">Confirm delivery to let the manufacturer know and release payment.</p>
                    <button
                      onClick={() => {
                      ...
                      disabled={confirming}
                      className="w-full py-3 rounded-xl gradient-card-purple text-white font-black text-sm flex items-center justify-center gap-2 disabled:opacity-50 hover:opacity-90 transition-all"
                    >
