 ...
 */
async function getOrCreateManufacturer(userId) {
 ...
 */
async function getOrCreateBuyer(userId) {
...
router.get('/manufacturers/:id/payment-settings', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
...
router.put('/manufacturers/:id/payment-settings', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
    const userId = req.params.id;
    if (req.user.role !== 'admin' && req.user._id.toString() !== userId) {
      return errorResponse(res, 'Unauthorized access', 'FORBIDDEN', 403);
...
router.get('/manufacturers/:mfr_id/buyers/:buyer_id/terms', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
...
router.put('/manufacturers/:mfr_id/buyers/:buyer_id/terms', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
    const mfrUserId = req.params.mfr_id;
    const buyerUserId = req.params.buyer_id;
...
router.delete('/manufacturers/:mfr_id/buyers/:buyer_id/terms', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
    const mfrUserId = req.params.mfr_id;
    const buyerUserId = req.params.buyer_id;
...
router.get('/checkout/available-terms', protect, async (req, res) => {
  try {
...
router.get('/manufacturers/:id/receivables', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
    const userId = req.params.id;
    if (req.user.role !== 'admin' && req.user._id.toString() !== userId) {
      return errorResponse(res, 'Unauthorized access', 'FORBIDDEN', 403);
...
router.post('/payment-records/:id/mark-paid', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
...
router.post('/payment-records/:id/send-reminder', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
    const recordId = req.params.id;
...
router.get('/manufacturers/:id/reminders-log', protect, requireRole('manufacturer', 'admin'), async (req, res) => {
  try {
    const userId = req.params.id;
    if (req.user.role !== 'admin' && req.user._id.toString() !== userId) {
      return errorResponse(res, 'Unauthorized access', 'FORBIDDEN', 403);
